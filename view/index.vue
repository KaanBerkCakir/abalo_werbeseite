...
<div id="container" class="column">
    <div id="navbar" class="row al-end">
        ...
        <button v-if="!toolbar.signedIn" id="user-button" @click="userInteraction(true)"><i class="fas fa-sign-out-alt"></i></button>
    ...
    <div class="grow-1 row">
            ...
            <div id="menu" class="column al-s-center">
                <template v-for="(item, itemIndex) in menu.items">
                    ...
                    <span v-for="(subitem, subitemIndex) in item.subitems" :id='"subitem" + itemIndex + subitemIndex'
                          ...
                          @click="chooseMenu(itemIndex + subitemIndex)">{{subitem}}</span>
                </template>
            </div>
        ...
        <div id="content" class="card grow-1 column al-center">
            <span v-if="toolbar.signedIn && lists.cart.length === 0" id="shopping-card-null">Der Warenkorb ist leer.</span>
            <div v-else-if="toolbar.signedIn && lists.cart.length > 0" id="shopping-card-list" class="al-s-stretch column al-s-stretch">
                ...
                <ul id="all-articles-ul">
                    <template v-for="elem in lists.cart">
                        ...
                        <span>{{elem.ab_name}}</span>
                            ...
                            <div>
                                <button class="btn" onclick="handleRemove(elem.id)"><i class="fas fa-minus-circle" style="color: #bc2d2d"></i></button>
                                </div>
                            </div>
                        </li>
                    </template>
